...
/**
 * Generates a sequence of characters based on a character distribution model.
 *
 * This function creates an iterator that generates characters by sampling from
 * probability distributions associated with each character. It starts with a
 * seed character '.' and continues generating the next character based on the
 * current character's distribution until a '.' (end-of-sequence marker) is encountered.
 *
 * @param charDist A map where each character is associated with an
 *                 EnumeratedIntegerDistribution that defines the probability
 *                 distribution of the next character to generate
 *
 * @return An Iterator[Char] that yields generated characters until the end-of-sequence
 *         marker '.' is sampled, at which point iteration terminates
 *
 * @example
 *   {{{
 *     val distribution: Map[Char, EnumeratedIntegerDistribution] = ...
 *     val chars: Iterator[Char] = generator(distribution)
 *     val word: String = chars.mkString
 *   }}}
 */
